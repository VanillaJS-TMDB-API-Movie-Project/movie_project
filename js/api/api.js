...
export async function getTopRatedMovieArray(page) {
  ...
  for (let i = 0; i < 20; i++) {
    movie = { ...result[i] };
    movie["backdrop_path"] =
      "https://image.tmdb.org/t/p/original/" + result[i]["backdrop_path"];
    movie["poster_path"] =
...
export async function getCountryArray() {
  ...
  result = await fetch(
    "https://api.themoviedb.org/3/configuration/countries?language=ko",
    options
  )
    .then((response) => response.json())
    ...
    .catch((err) => console.error(err));
  {
    ...
    for (let i = 0; i < movie["production_companies"].length; i++) {
      ...
      for (let j = 0; j < countryArray.length; j++) {
        if (
          movie["production_companies"][i]["origin_country"] ===
          countryArray[j]["iso_3166_1"]
        ) {
          movie["production_companies"][i]["origin_country"] =
            countryArray[j]["native_name"];
        }
      }
    }
    ...
    for (let i = 0; i < movie["production_countries"].length; i++) {
      for (let j = 0; j < countryArray.length; j++) {
        if (
          movie["production_countries"][i]["iso_3166_1"] ===
          countryArray[j]["iso_3166_1"]
        ) {
...
export async function getKeywordArray(id) {
  ...
  result = await fetch(
    `https://api.themoviedb.org/3/movie/238/keywords`,
    options
  )
    .then((response) => response.json())
...
export async function getRecommendationArray(id, page) {
  ...
  result = await fetch(
    `https://api.themoviedb.org/3/movie/${id}/recommendations?language=ko&page=${page}`,
    options
  )
    .then((response) => response.json())
    .then((response) => response["results"])
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    if (isoCode[recommendation["original_language"]]) {
      recommendation["original_language"] =
        isoCode[recommendation["original_language"]];
    }
...
export async function getSimilarArray(id, page) {
  ...
  result = await fetch(
    `https://api.themoviedb.org/3/movie/${id}/similar?language=ko&page=${page}`,
    options
  )
    .then((response) => response.json())
    .then((response) => response["results"])
...
export async function getSearchArray(keyword, page) {
  ...
  result = await fetch(
    `https://api.themoviedb.org/3/search/movie?query=${keyword}&language=ko&page=${page}`,
    options
  )
    .then((response) => response.json())
    .then((response) => response["results"])
